require 'spec_helper'

describe PasswordResetsController do
  describe "GET show" do
    context "with valid token" do
      
      let!(:valid_user) { object_generator(:user, password_token: '123456789') }
      before { get :show, id: '123456789' }
      
      it "renders show template" do
        expect(response).to render_template :show
      end
      
      it "sets @token" do
        expect(assigns(:password_token)).to eq('123456789')
      end
    end 
    
    context "with invalid token" do
      it "redirects to expired password token page" do
        get :show, id: '123456789'
        expect(response).to redirect_to expired_password_token_path
      end
    end 
  end
  
  describe "POST create" do
    context "with valid token" do
      
      context "with password that fails validation" do
        let!(:valid_user) { object_generator(:user, password_token: '123456789', password: 'old_password') }
        before { post :create, password_token: '123456789', password: 'tiny' }
        
        it "renders the password_resets show template" do
          expect(response).to render_template :show
        end
        
        it "does not reset the password" do
          expect(valid_user.password).to eq('old_password')
        end
        
        it "shows error messages" do
          expect(valid_user.errors).not_to be_nil
        end
        
        it "assigns @user" do
          expect(assigns(:user)).to eq(valid_user)
        end
        
        it "assigns @token" do
          expect(assigns(:password_token)).to eq('123456789')
        end
      end
      
      context "with password that passes validation" do
        let!(:valid_user) { object_generator(:user, password_token: '123456789', password: 'old_password') }
        before { post :create, password_token: '123456789', password: 'new_password' }
      
        it "redirects to the sign in page" do
          expect(response).to redirect_to sign_in_path
        end
      
        it "updates the users password" do
          expect(valid_user.reload.authenticate('new_password')).to be_truthy
        end 
      
        it "sets a successful flash message" do
          expect(flash[:success]).to be_present
        end
      
        it "deletes the users token" do
          expect(valid_user.reload.password_token).not_to be_present
        end
      end
    end
    
    context "with invalid token" do
      it "redirects to the expired password token path" do
        post :create, id: '123456789', password: 'new_password'
        expect(response).to redirect_to expired_password_token_path
      end
      
    end
  end
end